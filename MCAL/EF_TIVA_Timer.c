...
extern volatile  U8_t gStopOperations_Flag ;
volatile U32_t gCounterEvery_3Hours  = 0;
U32_t gCounterExpired_3Months = 0;
...
void EF_void_Timer_PORTinit(void)
{
	...
	//Global interrupt
	 ...
	 EF_BOOLEAN_EEPROM_ReadNBytes (&gCounterExpired_3Months  , TIMER_EXPIRED_3Months_EEPROM_ADD , 4);

	 if (gCounterExpired_3Months >= TIMER_90DAY_PER_3HOURS)
	 {
	     gStopOperations_Flag = TRUE;
	 }
     ...
     if (gStopOperations_Flag == FALSE)
     {
         if (gCounterEvery_3Hours >= TIMER_EXPIRED_3_HOURS)
         {
             ...
             else
             {
                 ...
                 gCounterExpired_3Months++ ;
                 EF_BOOLEAN_EEPROM_WriteNBytes(&gCounterExpired_3Months  , TIMER_EXPIRED_3Months_EEPROM_ADD , 4);
             }
         }
         else
         {
